...
async function main() {
  ...
  await page.getByRole('button', { name: /Auto Triage/i }).click()
  await page.getByText(/Triage suggests|auto_triage completed|FINANCIAL/i).first().waitFor({ timeout: 15000 })
  ...
  await page.getByPlaceholder(/Search or run/i).waitFor({ timeout: 5000 })
