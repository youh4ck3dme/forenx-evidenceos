...
import type { RelationshipRecord, SubjectRecord } from '@/lib/storage/types'
...
export function NetworkGraph({
  ...
  return (
    <svg
      ...
      className="h-auto w-full rounded-sm border border-fx-border bg-fx-bg"
      role="img"
      aria-label="Malte network graph"
    >
      {edges.map((e) => {
        const hi =
          pathSet.has(e.from.id) && pathSet.has(e.to.id)
        return (
          <line
            key={e.id}
            x1={e.from.x}
            ...
            x2={e.to.x}
            y2={e.to.y}
            stroke={hi || e.chain ? 'var(--color-fx-accent)' : 'var(--color-fx-border)'}
        ...
        return (
          ...
          >
            <circle
              ...
              fill={
                node.shell
                  ? 'color-mix(in oklab, var(--color-fx-danger) 55%, transparent)'
                  : 'color-mix(in oklab, var(--color-fx-elevated) 90%, transparent)'
              }
              stroke={
                node.shell
                  ? 'var(--color-fx-danger)'
            ...
            <text
              y={24}
              textAnchor="middle"
              className="fill-[var(--color-fx-muted)]"
              style={{ fontSize: 9 }}
            >
            ...
            {node.score > 0 && (
              <text
                y={4}
                textAnchor="middle"
                className="fill-[var(--color-fx-text)]"
      ...
      {nodes.length === 0 && (
        <text
          ...
          y={height / 2}
          textAnchor="middle"
          className="fill-[var(--color-fx-dim)]"
          style={{ fontSize: 12 }}
        >
