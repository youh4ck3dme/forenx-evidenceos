...
import { cn, createId, formatDateTime } from '@/lib/utils/cn'
import {
...
export function MalteWorkspace({
  caseId,
  workspaceId,
}: {
  caseId: string
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-fx-bg">
      <header className="flex flex-wrap items-center gap-2 border-b border-fx-border px-3 py-2">
        <div className="mr-auto">
          <div className="font-mono text-[10px] tracking-[0.2em] text-fx-dim uppercase">
            Malte triage
          </div>
          <div className="text-xs text-fx-muted">
        ...
        <Button size="sm" disabled={busy || !transactions.length} onClick={() => void runDetection()}>
          Run detection
        </Button>
      ...
      {statusMsg && (
        <div className="border-b border-fx-border px-3 py-1.5 font-mono text-[11px] text-fx-accent">
          {statusMsg}
        </div>
      )}
      ...
      {view === 'weights' && (
        <ScrollArea className="min-h-0 flex-1 p-4">
          <h3 className="text-sm font-semibold text-fx-text">Configurable rule weights</h3>
          ...
          <div className="mt-4 grid max-w-lg gap-3">
            {(Object.keys(weights) as (keyof DetectionWeights)[]).map((key) => (
              <label key={key} className="flex items-center justify-between gap-3 text-xs">
                <span className="font-mono text-fx-muted">{key}</span>
                <input
                  type="number"
                  min={0}
                  max={100}
                  className="w-24 rounded-sm border border-fx-border bg-fx-elevated px-2 py-1 text-fx-text"
                  value={weights[key]}
                  onChange={(e) =>
      ...
      {view === 'graph' && (
        <div className="flex min-h-0 flex-1 flex-col gap-2 p-3">
          <div className="flex flex-wrap gap-2 text-xs">
            <select
              className="rounded-sm border border-fx-border bg-fx-elevated px-2 py-1 text-fx-text"
              value={pathFrom}
          ...
          {selectedSubjectId && (
            <SubjectCard
              ...
              txs={transactions.filter(
                (t) =>
                  t.fromSubjectId === selectedSubjectId ||
                  t.toSubjectId === selectedSubjectId,
              )}
            />
          )}
        </div>
      ...
      {view === 'alerts' && (
        <div className="flex min-h-0 flex-1 overflow-hidden">
          <div className="flex w-full min-w-0 flex-col border-r border-fx-border md:w-[42%]">
            <div className="flex gap-2 border-b border-fx-border p-2">
              <select
                className="flex-1 rounded-sm border border-fx-border bg-fx-elevated px-2 py-1 text-xs text-fx-text"
            ...
            <ScrollArea className="min-h-0 flex-1">
              {filtered.length === 0 ? (
                <p className="p-4 text-xs text-fx-muted">
                  No alerts yet. Import a statement, then run detection.
                </p>
              ) : (
                <ul className="divide-y divide-fx-border">
                  {filtered.map((alert) => (
                    <li key={alert.id}>
                      <button
                        type="button"
                        className={cn(
                          'flex w-full flex-col gap-0.5 px-3 py-2.5 text-left hover:bg-fx-elevated',
                          selected?.id === alert.id && 'bg-fx-elevated',
                        )}
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <span className="ml-auto font-mono text-[10px] text-fx-dim">
                            {alert.status}
                          </span>
                        </div>
                        <div className="text-xs font-medium text-fx-text">{alert.title}</div>
                      </button>
                    </li>
                  ))}
                </ul>
              ...
              <div className="space-y-4 p-4">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="font-mono text-sm text-fx-accent">
                      {selected.score}/100
                    </span>
                    <span className="font-mono text-[10px] text-fx-dim">
                  ...
                  <h3 className="mt-2 text-sm font-semibold text-fx-text">{selected.title}</h3>
                  <p className="mt-1 text-xs text-fx-muted">{selected.description}</p>
                  <p className="mt-1 font-mono text-[10px] text-fx-dim">
                    {formatDateTime(selected.createdAt)}
                  </p>
                </div>

                <div>
                  <h4 className="font-mono text-[10px] tracking-wider text-fx-dim uppercase">
                    Score breakdown
                  </h4>
                  <ul className="mt-2 space-y-2">
                    {selected.factors.map((f, i) => (
                      <li
                        key={`${f.code}-${i}`}
                        className="rounded-sm border border-fx-border bg-fx-panel px-3 py-2"
                      >
                        <div className="flex justify-between gap-2 text-xs">
                          <span className="font-medium text-fx-text">{f.label}</span>
                          <span className="shrink-0 font-mono text-fx-accent">
                ...
                {selected.transactionIds.length > 0 && (
                  <TxList
                    items={transactions.filter((t) =>
                      selected.transactionIds.includes(t.id),
                    )}
                  />
                )}
              </div>
            ) : (
              <p className="p-4 text-xs text-fx-muted">Select an alert to see score factors.</p>
            )}
          </ScrollArea>
        </div>
      )}
...
function SeverityChip({ severity }: { severity: AlertSeverity }) {
  const color =
    severity === 'CRITICAL'
      ? 'text-fx-danger border-fx-danger/40'
      : severity === 'HIGH'
        ? 'text-fx-warn border-fx-warn/40'
        : severity === 'MEDIUM'
          ? 'text-fx-accent border-fx-accent/40'
          : 'text-fx-muted border-fx-border'
  return (
...
function SubjectCard({
  ...
  return (
    <div className="rounded-sm border border-fx-border bg-fx-panel p-3 text-xs">
      <div className="font-semibold text-fx-text">
        {subject.name}{' '}
        <span className="font-mono text-fx-dim">
  ...
  return (
    <div>
      ...
      <ul className="mt-1 space-y-1">
        {items.map((t) => (
          <li key={t.id} className="font-mono text-[11px] text-fx-muted">
